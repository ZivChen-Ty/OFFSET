    ...
    def get_test_data(self, name):
        ...
        if self.split == 'val-split':
            ...
            for id, image_name in enumerate(images):
                ...
                out['target_img_data_seg'] = self.get_saved_Seg(name + '_' + image_name, stage=1)      
                test_targets.append(out)
        return test_queries, test_targets


class Shoes_SavedSegment(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        out['source_img_data_seg'] = self.get_img(caption['source'].replace(".jpg", "-segmask.jpg"), 0)
        out['target_img_data'] = self.get_img(caption['target'], 0)
        out['target_img_data_seg'] = self.get_img(caption['target'].replace(".jpg", "-segmask.jpg"), 0)
    ...
    def get_img_Seg(self, image_name, caption, device, stage=0):
        ...
        image_features = (image_features - image_features.min()) / (image_features.max() - image_features.min())
        image_features_pil = PIL.Image.fromarray((image_features.detach().cpu().numpy() * 255).astype(np.uint8))
        image_features_pil.save(image_name.replace(".jpg", "-seg.jpg"))
        img = self.transform[stage](image_features_pil)

        seg_features = torch.sigmoid(preds).squeeze(1)
        seg_pil = PIL.Image.fromarray((seg_features.detach().cpu().numpy() * 255).astype(np.uint8))
        seg_pil.save(image_name.replace(".jpg", "-segmask.jpg"))
        seg_img = self.transform[stage](seg_pil)
        # img = self.transform[stage](img)
        return img, seg_img
    ...
    def get_test_queries(self):
        ...
        for idx in trange(len(self.eval_relative_pairs)):
            ...
            out['source_img_data_seg'] = self.get_img(candidate.replace(".jpg", "-segmask.jpg"), 1)
            out['target_img_id'] = self.imgimages_all.index(target)
            out['target_img_data'] = self.get_img(target, 1)
            out['target_img_data_seg'] = self.get_img(target.replace(".jpg", "-segmask.jpg"), 1)
            out['mod'] = {'str': mod_str}
...
class CIRR_SavedSegment(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        out['source_img_data_seg'] = self.get_img(self.train_image_path[reference_name].replace(".png", "-segmask.png"), 0)
        out['target_img_data'] = self.get_img(self.train_image_path[target_name], 0)
        out['target_img_data_seg'] = self.get_img(self.train_image_path[target_name].replace(".png", "-segmask.png"), 0)
        out['mod'] = {'str':mod_str}
        return out

    def get_img_Seg(self, image_name, caption, device, stage=0):
        img_path = os.path.join(self.path, image_name.lstrip('./'))
        with open(img_path, 'rb') as f:
            img = PIL.Image.open(f)
            img = img.convert('RGB')
        CLIPSeg_input = self.CLIPSeg_processor(text=[caption], images=[img], padding="max_length", return_tensors="pt")
        ...
        image_features = (image_features - image_features.min()) / (image_features.max() - image_features.min())
        image_features_pil = PIL.Image.fromarray((image_features.detach().cpu().numpy() * 255).astype(np.uint8))
        image_features_pil.save(img_path.replace(".png", "-seg.png"))
        img = self.transform[stage](image_features_pil)
    ...
    def get_val_queries(self):
        ...
        for idx in trange(len(val_data)):
            ...
            out['source_img_data_seg'] = self.get_img(val_image_path[reference_name].replace(".png", "-segmask.png"), 1)
            out['target_img_id'] = val_image_name.index(target_name)
            out['target_img_data'] = self.get_img(val_image_path[target_name], 1)
            out['target_img_data_seg'] = self.get_img(val_image_path[target_name].replace(".png", "-segmask.png"), 1)
            out['mod'] = {'str':mod_str}
    ...
    def get_test_queries(self):
        ...
        for i in trange(len(test_data)):
            ...
            out['reference_data_seg'] = self.get_img(test_image_path[caption['reference']].replace(".png", "-segmask.png"), 1)
        ...
        for i in trange(len(test_image_name)):
            ...
            image_data.append([data, data_seg])
